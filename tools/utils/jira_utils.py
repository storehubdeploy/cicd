...
# TODO: project_id, fix_version is necessary?
def get_ticket_info(issue_id):
    url = 'https://storehub.atlassian.net/rest/api/3/issue/{}'.format(issue_id)
    r = requests.get(url, auth=CONSTANTS.AUTH)
    res = r.json()
    # json_formatted_str = json.dumps(res, indent=2)
    # print(json_formatted_str)

    project_id = res['fields']['project']['id']
    assignee = res['fields']['assignee']['displayName']
    summary = res['fields']['summary']
    fix_version = ''
    if res['fields']['fixVersions']:
        fix_version = res['fields']['fixVersions'][0]['name']

    return assignee, summary, project_id, fix_version
