...
helm_pack() {
    ...
    helm fetch ${env}/${app}
    tar -xf ${app}*.tgz
    if [ ${app} = "sms-api" ] && [ ${cluster} = "alpha" ];then
        sed -i 's/30001/30002/g' ${app}/templates/service.yaml
    elif [ ${app} = "sms-api" ] && [ ${cluster} = "staging" ];then
        sed -i 's/30002/30001/g' ${app}/templates/service.yaml
    fi
    rm -f ${app}*.tgz
...
if [ $env = "uat" ];then
    ...
    #helm install --name=${app} --namespace=${cluster} ${env}/${app}
