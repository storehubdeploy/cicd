...
image_tag_json=`curl -s -u "admin:${HELM_PSW}" -X GET "https://harbor.mymyhub.com/api/repositories/fat/backoffice-v1-web/tags" -H "accept: application/json"`
...
# def 
main_init() {
    [ -e "${WORKSPACE}/.gitmodules" ] && git submodule update --init -f
    # check_schemas
    [ "$?" != 0 ] && exit 1
}
...
docker_part() {
    ...
    if [ $app = "beep-v1-web" ] || [ $app = "ist-v1-web" ];then
         python cicd/ci/libs/get_config.py --env ${env} --appid ${app} --cluster ${cluster}
    fi
    docker build -t ${DOCKER_REG}/${env}/backoffice-v1-web:${git_tag} . || die ">>> Docker Build Failed"
    rm -f ${WORKSPACE}/.env
    blue ">>> Docker push"
    docker push ${DOCKER_REG}/${env}/backoffice-v1-web:${git_tag} || die ">>> Docker Push Failed"
...
helm_pack() {
    ...
    tar -xf ${app}*.tgz
    rm -f ${app}*.tgz
    python3 ${script_path}/shared_yaml-bo.py -a ${app}  -e ${env} -c ${cluster} || die ">>> Helm Generate Yaml Failed"
