...
image_tag_json=`curl -s -u "admin:${HELM_PSW}" -X GET "https://harbor.mymyhub.com/api/repositories/pro/backoffice-v1-web/tags" -H "accept: application/json"`
...
helm_pack() {
    ...
    tar -xf ${app}*.tgz
    rm -f ${app}*.tgz
    python3 ${script_path}/generate_yaml-bo.py -a ${app}  -e ${env} -c ${cluster} || die ">>> Helm Generate Yaml Failed"
...
if [ $env = "pro" ];then
    ...
    #helm upgrade ${app} ${env}/${app} --namespace=${cluster} 
else
